...
package Test_TTY is
   ...
   procedure Test_Stream_Kind_Values (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-TTY-001: Stream_Kind positional values match fd convention
   procedure Test_Stream_Kind_Positions (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-TTY-006: TTY_Status record stores three independent Booleans
   procedure Test_TTY_Status_Record (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-TTY-002, FUNC-TTY-004: Is_TTY returns Boolean without raising
   procedure Test_Is_TTY_No_Exception (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-TTY-006: Query_All returns consistent results with Is_TTY
   procedure Test_Query_All_Consistency (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-TTY-002, FUNC-TTY-004: Is_TTY called multiple times is stable
   procedure Test_Is_TTY_Stability (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-TTY-006: TTY_Status aggregate construction
   procedure Test_TTY_Status_Aggregate (T : in out AUnit.Test_Cases.Test_Case'Class);
