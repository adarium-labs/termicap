...
package Test_Sigwinch is
   ...
   procedure Test_Install_Idempotent (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-SWC-001: Uninstall without prior Install is a no-op
   procedure Test_Uninstall_Without_Install_Is_No_Op (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-SWC-001, FUNC-SWC-005: Get_Pipe_Read_FD returns -1 after Uninstall
   procedure Test_Pipe_FD_Invalid_After_Uninstall (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-SWC-005: Get_Pipe_Read_FD returns >= 0 after Install
   procedure Test_Pipe_FD_Valid_After_Install (T : in out AUnit.Test_Cases.Test_Case'Class);
   ...
   procedure Test_Has_Resize_False_After_Install (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-SWC-003: Acknowledge_Resize is a no-op when no resize pending
   procedure Test_Acknowledge_Resize_No_Op_When_No_Pending (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-SWC-003, FUNC-SWC-008: Has_Resize returns False when not installed
   procedure Test_Has_Resize_False_When_Not_Installed (T : in out AUnit.Test_Cases.Test_Case'Class);
   ...
   procedure Test_Cached_Size_Valid_After_Install (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-SWC-006, FUNC-SWC-010: Get_Cached_Size when not installed returns
   --  80x24 default
   procedure Test_Cached_Size_Default_When_Not_Installed (T : in out AUnit.Test_Cases.Test_Case'Class);
   ...
   procedure Test_Pipe_FD_Is_Open_After_Install (T : in out AUnit.Test_Cases.Test_Case'Class);

   --  FUNC-SWC-006: Pipe FD is closed after Uninstall (fcntl returns error)
   procedure Test_Pipe_FD_Closed_After_Uninstall (T : in out AUnit.Test_Cases.Test_Case'Class);
