...
package body Test_Environment_Capture is
   ...
   begin
      Register_Routine (T, Test_Capture_Contains_System_Variable'Access,
         "Capture_Current: snapshot contains a known system variable");
      Register_Routine (T, Test_Capture_Absent_Variable_Returns_Empty'Access,
         "Capture_Current: absent variable returns empty string");
   end Register_Tests;
   ...
   begin
      ...
      Has_Known_Var :=
         Contains (Env, "PATH") or else Contains (Env, "HOME");

      Assert
         (Has_Known_Var,
          "Capture_Current: snapshot should contain at least one of "
          & "'PATH' or 'HOME' from the process environment");
