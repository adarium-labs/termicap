...
**Responsibility:** Probes the POSIX C library `wcwidth()` function on three sentinel codepoints to determine the Unicode version supported by the terminal's C runtime locale. Provides a `Wcwidth_Level` enumeration (`Unknown`, `Unicode_3`, `Unicode_13`, `Unicode_16`) that refines the coarser `Unicode_Level` result produced by `Termicap.Unicode`. Also provides the pure `Refine_Unicode_Level` function that combines both results via an upgrade-only `'Max` operation.

The package spec is SPARK-visible (`SPARK_Mode => On`) and contains all types, sentinel constants, and function specifications. The body carries `SPARK_Mode => Off` because it calls the C FFI bindings for `wcwidth()` and `setlocale()`. Platform dispatch follows ADR-0018: the POSIX body (`src/posix/`) performs the real probe; the Windows body (`src/windows/`) is a stub that returns `Unknown` unconditionally. A C helper `termicap_wcwidth.c` exposes `termicap_lc_ctype()` to provide the portable `LC_CTYPE` numeric value (which differs between Linux/macOS/FreeBSD).

| Property | Value |
|----------|-------|
| Files | `src/termicap-wcwidth.ads`, `src/posix/termicap-wcwidth.adb`, `src/windows/termicap-wcwidth.adb`, `src/c/termicap_wcwidth.c` |
| SPARK_Mode | On (spec), Off (both bodies) |
| Dependencies | `Termicap.Unicode`, `Interfaces.C` (POSIX body only) |

#### Key Types

| Type | Description |
|------|-------------|
| `Wcwidth_Level` | Four-literal enumeration: `Unknown` (probe inconclusive or not performed), `Unicode_3` (locale confirms at least Unicode 3.0 via U+28FF), `Unicode_13` (locale confirms at least Unicode 13.0 via U+1FB38), `Unicode_16` (locale confirms at least Unicode 16.0 via U+1CD00). Ordered: `Unknown < Unicode_3 < Unicode_13 < Unicode_16`; supports `'Max` for ceiling operations. |
| `Optional_Wcwidth_Level` | Discriminated record wrapping an optional `Wcwidth_Level` (`Is_Set : Boolean`). Used by the POSIX body to implement the per-process cache. Declared in the SPARK-visible spec so GNATprove can see its shape. |

#### Key Constants

| Constant | Value | Description |
|----------|-------|-------------|
| `WCW_SENTINEL_UNI3` | `16#28FF#` | U+28FF BRAILLE PATTERN DOTS-12345678 (Unicode 3.0). `wcwidth()` >= 1 confirms Unicode 3.0 locale support. |
| `WCW_SENTINEL_UNI13` | `16#1FB38#` | U+1FB38 UPPER LEFT BLOCK SEXTANT-2 AND 5 AND 6 (Unicode 13.0). `wcwidth()` >= 1 confirms Unicode 13.0 locale support. |
| `WCW_SENTINEL_UNI16` | `16#1CD00#` | U+1CD00 (Symbols for Legacy Computing Supplement, Unicode 16.0). `wcwidth()` >= 1 confirms Unicode 16.0 locale support. |

#### Public Operations

| Subprogram | Kind | SPARK Contract | Requirements |
|-----------|------|---------------|--------------|
| `Probe_Wcwidth_Level` | Function | Spec: SPARK On; Body: SPARK Off (FFI). Caches result after first call. Returns `Unknown` when locale is "C"/"POSIX", `setlocale()` returns NULL, all sentinels return <= 0, or platform is Windows. | FUNC-WCW-003, FUNC-WCW-006, FUNC-WCW-007, FUNC-WCW-010, FUNC-WCW-011, FUNC-WCW-012 |
| `Refine_Unicode_Level` | Function | `Global => null`; SPARK Silver provable. Returns `Unicode_Level'Max (Env_Level, To_Unicode_Level (Wcw_Level))`. `Unknown` maps to no change; `Unicode_3`/`Unicode_13` map to at least `Basic`; `Unicode_16` maps to at least `Extended`. | FUNC-WCW-005, FUNC-WCW-012 |

#### Probing Algorithm

`Probe_Wcwidth_Level` tests sentinels in descending Unicode version order for early exit (FUNC-WCW-003):

| Step | Action |
|------|--------|
| 0 | Cache hit check — return cached `Level` immediately if `Is_Set = True`. |
| 1 | Locale guard — call `setlocale(LC_CTYPE, NULL)`; if result is `"C"` or `"POSIX"` (or NULL), return `Unknown` without calling `wcwidth()`. |
| 2 | Probe `WCW_SENTINEL_UNI16` (U+1CD00, Unicode 16): `wcwidth()` >= 1 → return `Unicode_16`. |
| 3 | Probe `WCW_SENTINEL_UNI13` (U+1FB38, Unicode 13): `wcwidth()` >= 1 → return `Unicode_13`. |
| 4 | Probe `WCW_SENTINEL_UNI3` (U+28FF, Unicode 3): `wcwidth()` >= 1 → return `Unicode_3`. |
| 5 | All probes non-positive — return `Unknown`. |

#### Thread Safety

`Probe_Wcwidth_Level` is safe to call from multiple threads provided no thread is changing the locale (`setlocale()`) concurrently. The recommended usage is to call it once at process startup, before spawning application threads (FUNC-WCW-009). The result cache is implemented as a protected object (`Wcwidth_Cache`) in the POSIX body, making first-call initialisation safe against concurrent callers. The `wcwidth()` and `setlocale()` FFI calls themselves are not protected (they are outside the protected region) because POSIX does not require them to be serialised and holding a lock during a C library call would create unnecessary serialisation.

#### Relationship to Other Packages

`Termicap.Wcwidth` depends on `Termicap.Unicode` (for the `Unicode_Level` type used in `Refine_Unicode_Level`) and has no dependency on `Termicap.Environment` (the probe reads C locale state, not the environment snapshot) or `Termicap.TTY` (the probe does not require a TTY). It is a sibling of `Termicap.Unicode`, not a child, following the flat naming convention of all other Termicap feature packages (ADR-0032). The typical caller sequence is: `Detect_Unicode_Level (Env)` → `Probe_Wcwidth_Level` → `Refine_Unicode_Level`.
...
│   └─────────────────────────────────────────────┘  │
│                                                     │
│   Termicap.Wcwidth (spec only)                      │
│   ┌─────────────────────────────────────────────┐  │
│   │  Wcwidth_Level, Optional_Wcwidth_Level types│  │
│   │  WCW_SENTINEL_UNI3/13/16 constants          │  │
