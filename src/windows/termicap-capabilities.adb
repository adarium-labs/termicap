-------------------------------------------------------------------------------
--  Termicap.Capabilities - Terminal Capability Record Assembly (Windows Body)
--
...
--  @description
--  Windows body for Termicap.Capabilities.  The assembly logic is
--  platform-agnostic; this body delegates to the same sub-detectors used on
...
is
   ...
   ---------------------------------------------------------------------------
   --  Cache — protected object for lazy per-stream caching (FUNC-CAP-008)
   ---------------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------------
   --  Detect (FUNC-CAP-004) — fresh, uncached detection
   ---------------------------------------------------------------------------
   ...
   begin
      ...
      declare
         --  Check for any override that would supersede Win32-based detection.
         --  The protected override state takes precedence over env-var heuristics.
         Has_Override : constant Boolean :=
            Termicap.Override.Get_Override /= Termicap.Override.Auto
            or else Termicap.Environment.Contains (Env, "NO_COLOR")
            or else Termicap.Environment.Contains (Env, "FORCE_COLOR");
         Win32_Level  : constant Termicap.Color.Color_Level :=
            (if Is_TTY_For_Stream and then not Has_Override
             then Termicap.Win32_Color.Detect_Windows_Color_Level (Env)
             else Termicap.Color.None);
         Env_Level    : constant Termicap.Color.Color_Level :=
            Termicap.Color.Detect_Color_Level (Env, Is_TTY_For_Stream);
      begin
         Color := Termicap.Color.Color_Level'Max (Win32_Level, Env_Level);
      end;
   ...
   ---------------------------------------------------------------------------
   --  Get (FUNC-CAP-003) — lazy cached detection
   ---------------------------------------------------------------------------
