...
package body Termicap.Color.Detection is
   ...
   ---------------------------------------------------------------------------
   --  Internal helper shared by both detection functions.
   --  Runs the OSC query for Kind, then falls back to the COLORFGBG env var.
   --  For the COLORFGBG fallback, Background kind reads CFGBG.Background and
   --  Foreground kind reads CFGBG.Foreground.
   ---------------------------------------------------------------------------

   function Detect_Color (Kind : BG_Query.Query_Kind; Timeout_Ms : Natural) return Detection_Result is
      use type BG_Query.Query_Kind;
               ...
               begin
                  if CFGBG.Success then
                     return
                       (Success => True,
                        Color   =>
                          BG_Query.Ansi_To_RGB
                            (if Kind = BG_Query.Background then CFGBG.Background else CFGBG.Foreground));
                  end if;
               end;
            end if;
         end;
