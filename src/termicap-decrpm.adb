...
package body Termicap.DECRPM is
   ...
   begin
      ...
      if Temp = 0 then
         ...
         while Temp > 0 loop
            ...
            Rev_Buf (Rev_Count) :=
              Byte (Character'Pos ('0') + (Temp mod 10));
            Temp := Temp / 10;
         end loop;
   ...
   begin
      ...
      --  Check prefix: ESC [ ?
      if Bytes (Bytes'First)     /= 16#1B# or else
         Bytes (Bytes'First + 1) /= 16#5B# or else
         Bytes (Bytes'First + 2) /= 16#3F#
      then
         return False;
      end if;
      ...
      I          := Bytes'First + 3;
      Mode_Count := 0;
      while I <= Bytes'First + Length - 1 and then
            Bytes (I) >= 16#30# and then Bytes (I) <= 16#39#
      loop
   ...
   is
      Default_Report : constant Mode_Report :=
        (Mode => 0, Status => Not_Recognized);
      I              : Natural;
      Ps             : Natural := 0;
      Pm             : Natural := 0;
